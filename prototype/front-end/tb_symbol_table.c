...
struct symbol *lookup(char* sym){  
	...
	int scount = NHASH; /* how many have we looked at */  		while(--scount >= 0) {    
			...
			if(++sp >= symtab+NHASH) sp = symtab; /* try next entry */  
		}  
...
/* print the references * sort the table alphabetically * then flip each entry's reflist to get it into forward order * and print it out  *//* aux function for sorting */
...
void printrefs(){  
	...
	for(sp = symtab; sp->name && sp < symtab+NHASH; sp++) {    
		char *prevfn = NULL; /*last printed filename*/ 
